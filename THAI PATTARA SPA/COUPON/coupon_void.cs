...
{
    public partial class coupon_void : Form
    {
        public coupon_void()
        {
            InitializeComponent();
        ...
        private void manage_btn_Click(object sender, EventArgs e)
        {
            ...
            FROM COUPON A
            INNER JOIN BILL_DISCOUNT B ON B.USE_CARD_TYPE = 3 AND A.COUPON_ID = B.USE_CARD_ID
            ...
            GF.showLoading(this);
            DB.beginTrans();
            queryString = "UPDATE COUPON SET IS_VOID = 1, VOIDED_BY = " + GF.emp_id + ", VOIDED_REASON = '" + reason.Text.Trim() + "', VOIDED_DATETIME = CURRENT_TIMESTAMP, IS_USE = 0 WHERE CONVERT(BIGINT, CARD_NO) BETWEEN " + code_begin.Text.Trim() + " AND " + code_end.Text.Trim();
            if (!DB.set(queryString, "VOID COUPON"))
            {
                MessageBox.Show("ERROR VOID COUPON(s) !!", "ERROR");
                GF.closeLoading();
                return;
            }
            DB.close();
        ...
        private void coupon_void_FormClosed(object sender, FormClosedEventArgs e)
        {
